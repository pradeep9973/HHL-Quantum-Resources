...
def plot_sigmas_ggm_error(file_names):
    plt.rcParams["text.usetex"] = True
    fig, ax = plt.subplots(figsize=(6, 3.6))
    ax.grid(True)
    ...
    ax.set_ylabel(r'$\langle E_r \rangle$', fontsize=18, weight='bold')

    ax2 = ax.twinx()
    ax2.set_ylabel(r"$\langle \mathcal{E} \rangle$", fontsize=18, rotation=90)
    ...
    # # ax.set_title(r'\textbf{Entanglement in HHL Algorithm}')
    # ax.set_xlabel(r"$b_{0}^{2}$",  fontsize=18,weight='bold')
    # ax.set_ylabel(r'$E_r$',  fontsize=18, weight='bold')
    # ax2 = ax.twinx()
    # ax2.set_ylabel(r"$\langle \mathcal{E} \rangle$", fontsize=18, rotation=90)
...
###########################
    ...
    for f in file_names:
        ...
        y1 = sd
        # y2 = sp
        ax.plot(x, y1, label=r'$\sigma$ = 0.'+str(float(f[-5:-4])*2.0)[:1] )
        ax2.plot(x,y)
...
def plot_sigmas_ggm_error_new():
    ...
    # ax.set_title(r'\textbf{Entanglement in HHL Algorithm}')
    ax.set_xlabel(r"$b_{0}^{2}$", fontsize=18, weight='bold')
    ax.set_ylabel(r'$\langle \mathcal{E} \rangle$', fontsize=18, weight='bold')

    ax2 = ax.twinx()
    ax2.set_ylabel(r"$\langle E_r \rangle$", fontsize=18, rotation=90)
    ...
    files_error = ["Data_files/disorder/error_0_1.txt","Data_files/disorder/error_0_2.txt","Data_files/disorder/error_0_4.txt"]

    GGM = []
    log_neg =[]
    C_r = []
    for file in range(3):
        f = open(files_entanglement[file],"r")
        f_error = open(files_error[file],"r")
        csv_reader = csv.reader(f)
        csv_reader_error = csv.reader(f_error)
        b0 = []
        b0_sq = []
        ggm =[]
        cr =[]
        ...
        for row in csv_reader_error:
            error.append(float(row[1]))
        GGM.append(ggm)
        log_neg.append(ln)
        C_r.append(cr)
        ax.plot(b0_sq,ggm, label=r'$\sigma$ = 0.'+ files_entanglement[file][-5:-4],linestyle="-")
        ax2.plot(b0_sq,error,linestyle=":")
        f.close()
    ...
    num_ticks = 4  # Set the desired number of ticks
